...
def init_db_transcripciones(db_path=DB_OUTPUT):
    with sqlite3.connect(db_path) as conn:
        cursor = conn.cursor()
        # 🟩 Se agrega la columna 'model' entre filename y transcription
        cursor.execute("""
...
def listar_audios_disponibles():
    ...
    if not registros:
        print("⚠️ No hay audios registrados.")
        return []
...
def obtener_audios_por_id(ids):
    with sqlite3.connect(DB_INPUT) as conn:
        ...
        query = f"SELECT id, filename, audio, avg_rms_voz FROM grabaciones WHERE id IN ({placeholders})"
...
def transcribir_y_guardar(audios):
    ...
    for idx, (audio_id, filename, audio_blob, avg_rms_voz) in enumerate(audios, 1):
        ...
        print(f"\n[{idx}/{len(audios)}] Transcribiendo: {filename} (modelo: {MODEL})")  # 🟩 muestra el modelo
        ...
        # 🟩 Inserta también el modelo en la BD
        with sqlite3.connect(DB_OUTPUT) as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (filename, MODEL, texto, wer_info["wer"], cer_info["cer"], str(wer_info), str(cer_info), avg_rms_voz))
            conn.commit()

        wers.append(wer_info["wer"])
        cers.append(cer_info["cer"])
        rms_voz_vals.append(avg_rms_voz)

        print(f"✅ {filename}:")
        print(f"   → Modelo Whisper: {MODEL}")  # 🟩 NUEVO
        print(f"   → Transcripción: {texto}")
        print(f"   → WER: {wer_info['wer']:.2%} | CER: {cer_info['cer']:.2%}")
        print(f"   → RMS de voz: {avg_rms_voz:.4f}")

    # === RESUMEN FINAL ===
    if wers and cers:
        wer_mean, cer_mean = np.mean(wers), np.mean(cers)
        wer_std, cer_std = np.std(wers), np.std(cers)
        rms_mean = np.nanmean(rms_voz_vals)

        print("\n=== RESUMEN FINAL ===")
        print(f"Modelo usado: {MODEL}")  # 🟩 NUEVO
    ...
    if not audios_disponibles:
        ...
        if not seleccion:
            ...
            try:
                ...
                if not audios:
                    print("⚠️  No se encontraron audios con esos IDs.")
                else:
                    transcribir_y_guardar(audios)
                    print(f"\n✅ Transcripciones completadas y guardadas en '{DB_OUTPUT}'.")
